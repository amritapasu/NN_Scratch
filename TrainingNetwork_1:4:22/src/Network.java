...
{
   int numInputActivations; // NUM_CASES as a final ? but has to be made 25 then 5 ?, test with hyperbolic tangent
   ...
   public Network(String controlFile)
   {
      ...
      try
      {
         ...
         + "TrainingNetwork_1:4:22/src/" + mainControlFileName));
         ...
         inputs = new double[NUM_CASES][numInputActivations];
         ...
         {
            loadStoredInputs(t, input.nextLine());
         ...
         //input.nextLine();
         input.nextLine();
         trainingFile = new File("/Users/23amritap/Documents/vsCode/NN_2021/TrainingNetwork_1:4:22/src/"
                                       + input.nextLine());

         input.nextLine();
         input.nextLine();
         testingFile = new File("/Users/23amritap/Documents/vsCode/NN_2021/TrainingNetwork_1:4:22/src/"
                                       + input.nextLine());
    ...
    */
    public void loadStoredInputs(int t, String inputFileName)
    {
      try
      {
         Scanner in = new Scanner(new File(inputFileName));

         for (int p = 0; p < numInputActivations; p++)
         {
            inputs[t][p] = in.nextDouble()/255.0;
         }
   ...
   public void trainNetwork()
   {
      ...
      for (int t = 0; t < NUM_CASES; t++)     // evaluates the network for the first time with the original weights
         {
            ...
            System.out.println("before first training");
            implementNetworkTrain(activations[0]);
            System.out.println("after first training");
            for (int setNum = 0; setNum < numOutputActivations; setNum++)
            {
      ...
      while (iterations == 0 || (maxError > minSumError && iterations < maxIterations))
      {
         ...
         if (iterations % HUNDRED == 1)      // updates the training weights file every thousand iterations
         {
            updateTrainingWeightsFile();
            System.out.println(iterations + ": " + maxError);
         }
         iterations++;
   ...
   public void loadTestWeights()
   {
      try
      {
         ...
         for (int blankLine = 0; blankLine < 10; blankLine++)
         {
            input.nextLine();
         }
    ...
    */
    public void updateTrainingWeightsFile()
    {
      try
      {
         PrintWriter printWriter = new PrintWriter(new File("/Users/23amritap/Documents/vsCode/NN_2021/" + 
                                       "TrainingNetwork_1:4:22/src/trainingWeights_iteration" + iterations + ".txt"));

      printWriter.println("Network Configuration:");
      printWriter.println("\tInput Layer Activations: " + numInputActivations);
      printWriter.println("\t1st Hidden Layer Activations: " + numFirstHiddenActivations);
      printWriter.println("\t1st Hidden Layer Activations: " + numSecondHiddenActivations);
      printWriter.println("\tOutput Layer Activations: " + numOutputActivations);
      printWriter.println();

      printWriter.println("Maximum Calculated Error: " + maxError);
      printWriter.println("Lambda: " + lambda);
      printWriter.println("Random Weight Range: " + lowerBoundRand + " to " + upperBoundRand);
   ...
   public void implementNetworkTrain(double[] A)
   {
      ...
      for (int i = 0; i < numOutputActivations; i++)
      {
         ...
         for (int j = 0; j < numSecondHiddenActivations; j++)
         {
            //System.out.println("2nd " + System.currentTimeMillis());
            theta[1][j] = 0.0;
            for (int k = 0; k < numFirstHiddenActivations; k++)
            {
