 ...
 */
   public static void main(String[] args)
      ...
      int[][] croppedArray = new int[width][length];
// RBGQUAD
      int rgbQuad_rgbBlue;
...
*/
               ...
               for (int row = 0; row < bmpInfoHeader_biHeight; ++row) // read over the rows
                  ...
                  for (j = 0; j < iPelsPerRow; ++j)         // j is now just the column counter
                     ...
                     pel = dibdumper.rgbToPel(rgbQuad_rgbRed, rgbQuad_rgbGreen, rgbQuad_rgbBlue);
                     imageArray[i][j] = pel;

                     imageArray[i][j] = dibdumper.colorToGrayscale(imageArray[i][j]);
                     imageArray[i][j] = ~imageArray[i][j];
                     imageArray[i][j] = dibdumper.pelToRGB(imageArray[i][j]).red;
                     

                     mSum += (double)imageArray[i][j];
                     xmSum += (double)i * (double)imageArray[i][j];
                     ymSum += (double)j * (double)imageArray[i][j];
                     }
                  }
                  int xCom = (int)(xmSum/mSum);
                  int yCom = (int)(ymSum/mSum);
                  System.out.println("xCom: " + xCom);
                  System.out.println("yCom: " + yCom);
                  //System.out.println("image array 0 0: " + imageArray[0][0]);
                  ...
                  for (int x = 0; x < width; x++)
                  {
                     ...
                     {
                        croppedArray[x][y] = imageArray[x+xCom-(width/2)][y+yCom-(length/2)];
                        if (x == 0 && y == 0)
                        {
                           System.out.println("CA: " + croppedArray[x][y]);
                        }
                     }
                  }
                  */
                  ...
                  int xCounter = 0;
                  int yCounter = 0;
                  for (int x = xCom-cropDistance; x < xCom+cropDistance; x++)
                  {
                     //System.out.println("first " + x);
                     //System.out.println("xcount " + xCounter);
                     for (int y = yCom-cropDistance; y < yCom+cropDistance; y++)
                     {
                        croppedArray[xCounter][yCounter] = imageArray[x][y];
                        //System.out.println("second " + y);
                        //System.out.println("ycount " + yCounter);
                  ...
                  try
                  {
                     PrintWriter printWriter = new PrintWriter("/Users/23amritap/Documents/vsCode/NN_2021/Dibdump/croppedPels.txt");

                     for (int m = 0; m < width; m++)
                     {
                        for (int n = 0; n < length; n++)
                        {
                           printWriter.println(croppedArray[m][n] + ".0");
                        }
                        printWriter.println();
                        printWriter.println();
                     }
         ...
         {
         System.err.println("File input error here " + e);
         } 
/*
 * Console dump of image bytes in HEX if the image is smaller than 33 x 33
 */

System.out.printf("Conversion Complete\n");
 ...
 */
      try
         {
          bmpInfoHeader_biHeight = cropDistance*2;
          bmpInfoHeader_biWidth = cropDistance*2;
...
// there is no color table for this true color image, so write out the pels
         ...
         System.out.printf("Writing image %d x %d\n", length, width);
         ...
         for (i = bmpInfoHeader_biHeight - 1; i >= 0; --i)    // write over the rows (in the usual inverted format)
            ...
            for (j = 0; j < bmpInfoHeader_biWidth; ++j) // and the columns
               ...
               pel = dibdumper.rgbToPel(croppedArray[i][j], croppedArray[i][j], croppedArray[i][j]);
               //pel = croppedArray[i][j];
               if (i == 0 && j == 0)
               {
                  System.out.println("CA: " + croppedArray[i][j]);
               }
               ...
               //out.writeByte(croppedArray[i][j]);
